...
void CreateVBOFromObject(Objet * _monobjet) {
	...
	float * lnormales = new float[_monobjet -> nbsommets * 3];
	float * lsommets = new float[_monobjet -> nbsommets * 3];
	float * lcolors = new float[_monobjet -> nbsommets * 3];
	float * lelements = new float[_monobjet -> nbfaces * 3];

	//remplissage des tableaux
	//tableau des couleurs, 1 par point, tirée aléatoirement
	unsigned int cpt = 0;
	for (unsigned int i = 0; i < _monobjet -> nbsommets; i++) {
	...
	for (unsigned int i = 0; i < _monobjet -> nbsommets; i++) {
		...
		lsommets[cpt++] = _monobjet->lpoints[i].x;
		lsommets[cpt++] = _monobjet->lpoints[i].y;
		lsommets[cpt++] = _monobjet->lpoints[i].z;
	}

	//tableau des élements
	cpt=0;
	for (unsigned int i = 0; i < _monobjet -> nbfaces; i++) {
		lelements[cpt++] = _monobjet->lfaces[i].S1;
		lelements[cpt++] = _monobjet->lfaces[i].S2;
		lelements[cpt++] = _monobjet->lfaces[i].S3;
	}

	//VBO pour les sommets
	glGenBuffers((GLsizei) 1, &VBOSommets);
		glBindBuffer(GL_ARRAY_BUFFER, VBOSommets);
		glBufferData(GL_ARRAY_BUFFER, _monobjet -> nbsommets * 3 * sizeof(float), lsommets, GL_STATIC_DRAW);

	//VBO pour les couleurs
	glGenBuffers((GLsizei) 1, &VBOCouleurs);
		glBindBuffer(GL_ARRAY_BUFFER, VBOCouleurs);
		glBufferData(GL_ARRAY_BUFFER, _monobjet -> nbsommets * 3 * sizeof(float), lcolors, GL_STATIC_DRAW);

	//VBO pour les normales
	glGenBuffers((GLsizei) 1, &VBONormales);
		glBindBuffer(GL_ARRAY_BUFFER, VBONormales);
		glBufferData(GL_ARRAY_BUFFER, _monobjet -> nbsommets * 3 * sizeof(float) , lnormales, GL_STATIC_DRAW);

	//VBO pour les elements
	glGenBuffers((GLsizei) 1, &VBOElements);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, VBOElements);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, _monobjet -> nbfaces * 3 * sizeof(float) , lelements, GL_STATIC_DRAW);
...
void CreateVertexArraysFromObject(Objet* _monObjet) {
	...
	//création des tableaux de normales et points
	NormalArray = new float[_monObjet -> nbsommets * 3];
	VertexArray = new float[_monObjet -> nbsommets * 3];
	ElementArray = new float[_monObjet -> nbfaces * 3];
	ColorArray = new float[_monObjet -> nbsommets * 3];
	...
	for (unsigned int i = 0; i < _monObjet -> nbsommets; i++) {
	//On recopie UNE normale par sommets
		NormalArray[cpt++] = _monObjet -> lpointsnormales[i].x;
		NormalArray[cpt++] = _monObjet -> lpointsnormales[i].y;
		NormalArray[cpt++] = _monObjet -> lpointsnormales[i].z;
	}

	cpt = 0;
	//remplissage du tableau des sommets
	for (unsigned int i = 0; i < _monObjet -> nbsommets; i++) {
		VertexArray[cpt++] = _monObjet->lpoints[i].x;
		VertexArray[cpt++] = _monObjet->lpoints[i].y;
		VertexArray[cpt++] = _monObjet->lpoints[i].z;
	}

	//tableau des élements
	cpt=0;
	for (unsigned int i = 0; i < _monObjet -> nbfaces; i++) {
		ElementArray[cpt++] = _monObjet ->lfaces[i].S1;
		ElementArray[cpt++] = _monObjet ->lfaces[i].S2;
		ElementArray[cpt++] = _monObjet ->lfaces[i].S3;
	}
...
glEnableClientState(GL_ELEMENT_ARRAY_BUFFER);
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, VBOElements);
		...
		glDrawElements(GL_TRIANGLES, monobjet.nbsommets * 3, GL_UNSIGNED_INT, (void*)0);
