...
public:
	...
	// Différents accesseurs et mutateurs + surcharge opérateur ()
...
public:
	...
	static ColorImage* readJPEG(const char* fname); // Lecture dans un fichier JPEG

	void writeTGA(std::ofstream& f,bool rle=true) const; //Ecriture dans un fichier TGA
	static ColorImage* readTGA(std::ifstream& f); //Lecture dans un fichier TGA

	static ColorImage* readMaison(std::istream& f);
